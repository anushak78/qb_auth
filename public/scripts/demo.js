...
 
	...
	$("#save-q1").click(function (event) {
		event.preventDefault();
		// var state = editorStates['editor1'];
		// var data ="";
		// if (state) {
		// 	data = state.editor.getData();
		// }

		// var textToSave = data;//document.getElementById("editor1").value;
		...
		/* var textToSaveAsBlob = new Blob([textToSave], {type:"text/plain"});
		 ...
		 var fileNameToSaveAs = 'question';//document.getElementById("inputFileNameToSaveAs").value;
	  
	 ...
	  
		...
		for (var i = 0; i < editorIds.length; i++) {
			...
			var data1 = "";
			if (state) {
				data1 = state.editor.getData();
			}

			var textToSave = data1;
			...
			//	var data = lite.findPlugin(state.editor).getCleanText();
			//alert(data);
			if (state) {
			...
			if (i == 0) {
				cleanText = getURLParameter('id') + " : " + cleanText;
			}


			if (typeof (Storage) !== "undefined") {
				// Store
				var id = getURLParameter('id') + editorId;
				var cleanQ = id + "clean";
				localStorage.setItem(id, textToSave);
				localStorage.setItem(cleanQ, cleanText);
