(function () {
    ...
    function ShortFallCSQParentController($scope, $state, $filter, $window, userService, repositoryService, $sce, $http, $q) {

        $scope.newParentQuestion = repositoryService.getParentQuestion();
        if ($scope.newParentQuestion.length == 0) {
        ...
        $scope.saveCSQParentShortfall = function () {
            ...
            $scope.qst_type = window.localStorage.getItem("shortfall_question_qst_type");

            $scope.newParentQuestion.question = CKEDITOR.instances["editor1"].getData();
            $scope.newParentQuestion.userName = $scope.username;
            $scope.newParentQuestion.qb_id = $scope.qb_id;
            $scope.newParentQuestion.qb_pk = $scope.qb_pk;
            ...
            $scope.newParentQuestion.qst_type = $scope.qst_type;
            $scope.newParentQuestion.summary_no_of_question = $scope.childQuestionList.length
            ...
            if ($scope.error_msg.length > 0) {
                ...
                $http.post('/api/save_csq_parent_shortfall', $scope.newParentQuestion)
                    .then(function (response) {
                        alert("QBID " + $scope.newParentQuestion.qb_id + " Saved Successfully.")
                        repositoryService.setChildQuestionList({})
                        repositoryService.setParentQuestion({})
                        $scope.clearData()
        ...
        $scope.goBack = function () {
            if ($scope.childQuestionList.length > 0) {
                alert("Please save the shortfall before redirection")
            }
            else
                $state.go('vetterReviewQuestion');
        }
        ...
        $scope.savereplacementalldata = function () {
            ...
            $http.post('/api/save_csq_parent_in_vetter', parameters).then(function (response) {
                ...
                return $http.post('/api/add_child_question_vetter', $scope.newParentQuestion).then(function (response) {
        ...
        $scope.prepareAddChildQuestionData = function () {
            ...
            $state.go('add_shortfall_csq_child');
        };
    ...
    angular
        ...
        .controller('ShortFallCSQParentController', ShortFallCSQParentController);

    ShortFallCSQParentController.$inject = ['$scope', '$state', '$filter', '$window', 'userService', 'repositoryService',
        '$sce', '$http', '$q'];
