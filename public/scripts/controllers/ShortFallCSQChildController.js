(function () {
    ...
    function ShortFallCSQChildController($scope, $state, $filter, $window, userService, repositoryService, $http, $sce, $q, $timeout) {
        ...
        $scope.saveCSQShortFallChild = function () {
            var qb_id = window.localStorage.getItem("shortfall_csq_parent_qb_id");
        ...
        $scope.saveCSQChild = function () {
            ...
            if (($scope.newChildQuestion.marks != 0.5 && $scope.newChildQuestion.marks != 1 && $scope.newChildQuestion.marks != 2) || $scope.newChildQuestion.marks == null || $scope.newChildQuestion.marks == undefined) {
            ...
            var qb_id = window.localStorage.getItem("shortfall_csq_parent_qb_id");
            var qb_pk = window.localStorage.getItem("shortfall_csq_parent_qb_pk");
            $scope.exampaper_fk = window.localStorage.getItem("exampaper_pk");
            $scope.exam_fk = window.localStorage.getItem("exam_id");
            $scope.newChildQuestion.qbId = qb_id;
            $scope.newChildQuestion.exampaper_fk = $scope.exampaper_fk;
            $scope.newChildQuestion.exam_fk = $scope.exam_fk;
            $scope.newChildQuestion.qb_pk = qb_pk;

            var result = confirm("Are you sure you want to save child question?")
            if (result == true) {
                $http.post('/api/save_child_sf', $scope.newChildQuestion)
                    .then(function (response) {
                        $scope.updateChildCountsf();
                        repositoryService.addChildQuestion($scope.newChildQuestion)
                        alert('Data Saved Successfully.');
        ...
        $scope.updateChildCountsf = function () {
            var qb_id = window.localStorage.getItem("shortfall_csq_parent_qb_id");
            var qb_pk = window.localStorage.getItem("shortfall_csq_parent_qb_pk");
            $scope.exampaper_fk = window.localStorage.getItem("exampaper_pk");
            $scope.exam_fk = window.localStorage.getItem("exam_id");
            ...
            $http.post('/api/update_child_count_sf', $scope.newChildQuestion)
                .then(function (response) {
                });
        };
    ...
    angular
        ...
        .controller('ShortFallCSQChildController', ShortFallCSQChildController);

    ShortFallCSQChildController.$inject = ['$scope', '$state', '$filter', '$window', 'userService', 'repositoryService', '$http', '$sce', '$q', '$timeout'];
